...
class Vector:
    """
    Vector : a simple way to manipulate vector
    :param x: float | int
    :param y: float | int
    :param define_by_angle: bool
    :param angle: float (radians)
    :param norm: float | int
    """
    ...
    def __getitem__(self, item):
        if item == 0:
            ...
            print(f"[VECTOR] __getitem__ : Argument Error -> {item}")
            return 0
    ...
    def __add__(self, other):
        if isinstance(other, Vector):
            ...
            print(f"[VECTOR] __add__ : Argument Error -> {other}")
            return NullVector()
    ...
    def __mul__(self, other):
        if isinstance(other, (int, float)):
            ...
            print(f"[VECTOR] __mul__ : Argument Error -> {other}")
            raise ArgumentError
    ...
    def __sub__(self, other):
        if isinstance(other, Vector):
            ...
            print(f"[VECTOR] __sub__ : Argument Error -> {other} (type {type(other)})")
            return NullVector()
    ...
    def __truediv__(self, other):
        if isinstance(other, (int, float)):
            ...
            print(f"[VECTOR] __rtruediv__ : {self} cannot be divided by {other}")
            return NullVector()
    def __rtruediv__(self, other):
        print(f"[VECTOR] __rtruediv__ : {other} cannot be divided by a {self}")
        return NullVector()
    ...
    def __floordiv__(self, other):
        if isinstance(other, (int, float)):
            ...
            print(f"[VECTOR] __rfloordiv__ : {self} cannot be divided by {other}")
            return NullVector()
    def __rfloordiv__(self, other):
        print(f"[VECTOR] __rfloordiv__ : {other} cannot be divided by a {self}")
        return NullVector()
    ...
    def __eq__(self, other):
        if isinstance(other, Vector):
            ...
            print(f"[VECTOR] __eq__ : {other} cannot be compared to {self} -> False")
            return False
    ...
    def __call__(self, *args, **kwargs) -> list[int | float] | None:
        if len(args) == 0:
            ...
            if isinstance(x, (int, float)) and isinstance(y, (int, float)):
                ...
                print(f"[VECTOR] __call__ : error, x and y must be int or float")
        else:
            print(f"[VECTOR] __call__ : error, use Vector(new_x, new_y)")
        return None
    ...
    def dot(self, other):
        if isinstance(other, Vector):
            ...
            print(f"[VECTOR] dot() : incompatible dot product between : {self} {other} -> False")
            return 0

    def rotated(self, delta_angle, center=None, radians=True):
        # Translate vector to origin
        if center is None:
            center = Vector(0, 0)
        dx, dy = (self - center)()

        if radians is False: delta_angle = rad(delta_angle)

        cos_a = cos(delta_angle)
        sin_a = sin(delta_angle)

        # Apply rotation
        rotated_dx = dx * cos_a - dy * sin_a
        rotated_dy = dx * sin_a + dy * cos_a
        rotated_dp = Vector(rotated_dx, rotated_dy)
        return center + rotated_dp
